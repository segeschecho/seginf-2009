...
Si bien reporTool trae unos archivos de blacklists para ciertas categorias, es fácil modificar esas blacklist, o reemplazarlas por otras, crear un reporte nuevo que trabaje con blacklists de otras categorias.
...
Este reporte puede generar graficos de cantidad de infracciones por usuarios, que porcenteja del tráfico por usuario y total esta en infracción, cuales son los sitios de la blacklist mas visitados, entre otras cosas.

\subsection{Reporte de tráfico AJAX}
%TODO: sergio completa esto

\subsection{Reporte por content type}
En este reporte podemos ver el tipo de contenido del tráfico, basicamente hicimos seis separaciones distintas (audio, video, aplicación, multipart, imagen y texto). En el reporte podemos ver, por ejemplo, la cantidad de tráfico de tipo imagen y la cantidad de tráfico que no es imagen. También tenemos la cantidad de tráfico de imagen separada por usuario para poder ver mas en detalle que hace cada usuario.
...
Las blacklists solo encuentran requests a los dominios en ellos definidos, entonces es factible que un usuario pueda visitar alguna página que no esta en la blacklist pero que debería estarlo, o que incluso saltee el chequeo de las blacklists (por ejemplo navegando con la cache de google). La busqueda heuristica permite detectar estas cosas, al costo de posiblemente dar falsos positivos sobre el contenido de las visitas. Sin embargo puede ser una herramienta util para detectar usos indebidos.
...
Para poder definir un nuevo reporte, es necesario definir una clase que herede de Reporte. Esta misma debe implementar el metodo ejecutar que recibe un desde y un hasta, que son las fechas entre las cuales debe analizarse el tráfico.

Dicho metodo debe devolver un string latex, que es el que se utilizará para generar el pdf o el html correspondiente al reporte.
...
Las línes 3 y 4 son necesarias para importar la libreria traits que permite generar luego de forma automatica la GUI para modificar el reporte.

Luego comienza la definición de la clase. Las dos primeras lineas de la definicion, establecen dos atributos que va a tener el reporte, un flag booleano que por default vale True y un rango que va del 1 al 10 y que por default comienza en 5. Estos atributos se podran luego modificar mediante la interfaz grafica.
...
De esta forma el reporte queda agregado. Si se quiere agregar un reporte para una sesión nada mas, se puede utilizar la consola del modo interactivo. Sin modificar el código de reporTool usarse la funcion agregarReporte que recibe una instancia de reporte (en el ejemplo una instancia de ReporteTrucho) y el nombre que se le quiere dar al reporte, y lo agrega a la lista de reportes de la interfaz grafica. Este es un modo rápido de ir probando distintos reportes, los cuales se puede, incluso, definir en la consola.
...
Otra consideración que hay que hacer, es que al identificar a los usuarios por su IP un usuario malicioso podría inculpar a otro usuario inocente. Para esto no tendría mas que generar requests a sitios prohibidos por la blacklist spoofeando la IP de la victima. Entonces al correr reporTool, en el reporte aparecerá que la victima estuvo accediendo a sitios indebidos, cuando en verdad eso es falso.
